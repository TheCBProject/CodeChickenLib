...
public class QuadCornerKicker extends QuadTransformer {
    ...
    public boolean transform(Quad quad) {
        ...
        if (side == mySide || side == (mySide ^ 1)) return true;

        for (int hoz : horizonals[mySide]) {
            if (side == hoz || side == (hoz ^ 1)) continue;
            if ((facadeMask & (1 << hoz)) == 0) continue;

            Corner corner = Corner.fromSides(mySide ^ 1, side, hoz);
            for (Vertex vertex : quad.vertices) {
                double x = vertex.vec.x;
                double y = vertex.vec.y;
                double z = vertex.vec.z;
                if (!epsComp(x, corner.pX(box)) || !epsComp(y, corner.pY(box)) || !epsComp(z, corner.pZ(box))) continue;

                Vec3i vec = Direction.BY_3D_DATA[hoz].getUnitVec3i();
                x -= vec.getX() * thickness;
                y -= vec.getY() * thickness;
                z -= vec.getZ() * thickness;
                vertex.vec.set(x, y, z);
            }
        }
